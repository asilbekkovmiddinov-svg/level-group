...
ARENA_V5_SEASON_NAME = os.getenv("ARENA_V5_SEASON_NAME", "Haftalik Arena").strip()
ARENA_V5_SEASON_END_AT = os.getenv("ARENA_V5_SEASON_END_AT", "").strip()
...
ONCLICKA_REWARDED_AD_ENABLED = _env_bool("ONCLICKA_REWARDED_AD_ENABLED", False)


def onclicka_rewarded_ad_ready() -> bool:
    return (
        ONCLICKA_REWARDED_AD_ENABLED
        and len((ONCLICKA_REWARD_SECRET or "").strip()) >= 32
        and bool(ONCLICKA_SPOT_ID)
    )
