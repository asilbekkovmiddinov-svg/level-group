...
def send_admin_message(text: str, reply_markup: dict | None = None, chat_id: int | str | None = None) -> TelegramPhotoResult:
    ...
    request = urllib.request.Request(
        f"{config.TELEGRAM_API_BASE_URL.rstrip('/')}/bot{config.TELEGRAM_BOT_TOKEN}/sendMessage",
    ...
    if not isinstance(data, dict) or not data.get("ok") or not data.get("result", {}).get("message_id"):
