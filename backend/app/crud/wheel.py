...
COIN_130_INTERVAL_MIN = 48500
COIN_130_INTERVAL_MAX = 51500
...
BASE_REWARDS = [
    {
        "code": "lose",
        "type": REWARD_TYPE_NONE,
        "amount": Decimal("0"),
        "weight": 4500,
        "message": "❌ Bu safar yutuq chiqmadi.",
    },
    {
        "code": "bonus_spin",
        "type": REWARD_TYPE_BONUS_SPIN,
        "amount": Decimal("1"),
        "weight": 1000,
        "message": "🎁 Yana 1 marta bepul aylantirish yutdingiz!",
    },
    {
        "code": "efc_5",
...
def get_or_create_settings(db: Session):
    ...
    settings = WheelSettings(
        id=1,
        global_spin_count=0,
        next_130_coin_spin=random.randint(
            COIN_130_INTERVAL_MIN,
            COIN_130_INTERVAL_MAX,
        ),
        next_jackpot_spin=random.randint(
...
def create_spin_record(
    db: Session,
    telegram_id: int,
    spin_type: str,
    reward: dict,
    global_spin_number: int,
):
    spin = WheelSpin(
        telegram_id=telegram_id,
        spin_type=spin_type,
...
def spin_wheel(
    ...
    if not settings.is_active:
        return {
            "success": False,
            "message": "Wheel hozircha faol emas",
        }

    limit = get_or_create_daily_limit(
        db=db,
        telegram_id=telegram_id,
    )
