...
def accept_match(db: Session, match_id: int, opponent_telegram_id: int) -> Match:
    ...
    db.commit()
    db.refresh(match)from datetime import datetime, timedelta
from decimal import Decimal
from typing import Optional
...
def resolve_match(
    ...
    match.result_type = MatchResultType.TECHNICAL if match.status == MatchStatus.TECHNICAL_WIN else MatchResultType.NORMAL
